...
void Scheduler::Init() {
    ...
    // Attach VMs of all possible types to each machine
    ...
    // SimOutput(to_string(Machine_GetInfo(MachineId_t(0)).s_state), 0);
    // bool dynamic = false;
    // if(dynamic)
    ...
    // SimOutput("Scheduler::Init(): VM ids are " + to_string(vms[0]) + " ahd " + to_string(vms[1]), 0);
}
...
void Scheduler::NewTask(Time_t now, TaskId_t task_id) {
    ...
    TaskInfo_t new_task = GetTaskInfo(task_id);
    CPUType_t req_cpu = new_task.required_cpu;
    VMType_t req_vm = new_task.required_vm;
    switch(new_task.required_sla) {
        case SLA0:
            new_task.priority = HIGH_PRIORITY;
            break;
        case SLA1:
            new_task.priority = MID_PRIORITY;
            break;
        case SLA2:
            new_task.priority = MID_PRIORITY;
            break;
        case SLA3:
            new_task.priority = LOW_PRIORITY;
            break;
    }
    vector<VMId_t> possible_vms;
    switch(req_vm) {
        case LINUX_RT:
            possible_vms = linux_rt;
            break;
        case LINUX:
            possible_vms = linux;
            break;
    ...
    for(auto & vm: possible_vms) {
        if (VM_GetInfo(vm).cpu == req_cpu && vm_utilization[vm] <= 0.8) {
            VM_AddTask(vm, task_id, priority);
            vm_utilization[vm] += 0.1;
            break;
        }
    }
    sort(possible_vms.begin(), possible_vms.end(), [this](const VMId_t& a, const VMId_t& b)) {
        return vm_utilization[a] > vm_utilization[b];
    }
